...
export default function Dashboard() {
  ...
  const handleSave = async (college: any) => {
    ...
    const res = await apiPost("/save", { token, college });
    ...
    <div className="p-6 bg-gradient-to-br from-indigo-50 to-white min-h-screen">
      ...
      <div className="flex justify-between mb-6">
        ...
        <div className="flex gap-3">
          <button onClick={() => setShowSaved(!showSaved)} className="bg-black text-white px-4 py-2 rounded-xl">
            {showSaved ? "All" : "Saved"}
          </button>
      ...
      <div className="flex gap-3 mb-6">
        <select onChange={(e) => setLocation(e.target.value)} className="border p-3 rounded-xl">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {(showSaved ? savedData : filtered).map((c: any, i) => (
          <div key={i} className="bg-white p-5 rounded-2xl shadow-md">
